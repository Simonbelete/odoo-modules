...
class EmployeeDetail(models.Model):
    ...
    manufacturing_count = fields.Integer(compute="_manufacturing_count", store=True)
...
class PayrollContactDetail(models.Model):
    ...
    @api.depends('transport', 'incentive', 'house', 'phone_payroll', 'fuel', 'tip_confirmation')
    def _total_allowance(self):
        for total_data in self:
            total_data.total_allowance = total_data.transport + total_data.incentive \
                                         ...
                                         + total_data.fuel + total_data.tip_confirmation
...
class DeductionContactDetail(models.Model):
    ...
    mrp_production_contract_ids = fields.One2many('mrp.production', 'mrp_production_contract')
